...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    config.appKey = @"t131";
    config.secretKey = @"030b6feb60594eb28f84a3187954c9a9";
    config.vad = YES;
    config.frontTime = 30;
    config.backTime = 30;
    [SSOralEvaluatingManager registerEvaluatingManagerConfig:config];
    [[SSOralEvaluatingManager shareManager]registerEvaluatingType:OralEvaluatingTypeOffLine];
    
    self.window = [[UIWindow alloc]initWithFrame:[UIScreen mainScreen].bounds];
    ViewController *controller = [[ViewController alloc]init];
    self.window.rootViewController = controller;
    [self.window makeKeyAndVisible];
    NSLog(@"hoemDi:%@",NSHomeDirectory());
    return YES;
}
